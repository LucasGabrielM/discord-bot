...
export class ConfigDataService {
    get data() {
        const yaml_files = load(new YAMLParseService('./app.yml').execute());
        const old_data = load(new YAMLParseService('./app.yml').execute());
    ...
    checkConfig(res) {
        if (!res)
            throw new Error('Config file not contains any config, please check app.example.yml for example');
        if (!res.bot)
